...
import { updateRecurringTemplate } from '@/app/actions/recurring-invoices'
...
export default function EditRecurringInvoicePage() {
  ...
  useEffect(() => {
    async function loadData() {
      const [customersRes, projectsRes, templateRes] = await Promise.all([
      ...
      if (templateRes.data) {
        const t = templateRes.data as RecurringTemplate
        setTemplate(t)
        setName(t.name)
        ...
        setPaymentTerms(t.payment_terms_days)
        setNotes(t.notes || '')
        setStartDate(t.next_issue_date || new Date().toISOString().split('T')[0])
        setLines(
          t.lines.map((l) => ({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await updateRecurringTemplate(templateId, formData)
  ...
  if (initialLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <Loader2 className="h-8 w-8 animate-spin text-[rgba(232,236,255,0.5)]" />
      </div>
    )
  }
  ...
  if (!template) {
    return (
      <div className="py-12 text-center">
        <p className="text-[rgba(232,236,255,0.6)]">Template not found.</p>
        <Link
          href="/documents/recurring"
          className="mt-4 inline-block text-[13px] text-[#1f5bff] hover:underline"
        >
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div>
        ...
        <h1 className="text-2xl font-bold text-white">Edit Recurring Template</h1>
